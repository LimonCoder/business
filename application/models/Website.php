...
class Website extends CI_Model
{
    ...
    {
        return $this->db->get('aci_info')->result();
    }

    public function update_account($amount,$id){

        $acc_no = $this->db->where("id",$id)->get('aci_info')->row()->id;
        $total_amount = $this->db->where("id",$id)->get('aci_info')->row()->balance+$amount;
        $this->db->where("id",$acc_no)->update("aci_info",array("balance"=>$total_amount));

    }

    public function total_amount(){
        $this->db->select_sum('balance');
        $this->db->from('aci_info');
